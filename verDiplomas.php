...
function loadPage() {
    ...
    $msjTag = Session::message('diploma');
    $token = Token::generate();

    $resultsCount = $diploma->contarTotal('diplomas');
    $per_page = 3;
    ...
    $resultsToCast = $diploma->getOffLim($pagination->offset(), $pagination->_per_page, 'diplomas');
    $resultsToTemp = Diploma::arrConInfo($resultsToCast);

    $results = $incluibles->diplomas($resultsToTemp);
    
    $paginaTodas = <<<DIPLOMAS
        <!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
                ...
                <div class="mb-5">
                    <form action="./buscar.php" method="POST" id="form" autocomplete="off">
                        <div class="form-group">
                            <label for="busqueda">Ingrese el Alumno del Diploma a buscar</label>
...
DIPLOMAS;
    ...
    if ($paginationData['total'] > 0){
        ...
        if($pagination->_current_page >= 2){ 
            $pageBtn .= "<li class='page-item'><a class='page-link' href='verDiplomas.php?&pagina=1'> << </a></li>"; 
            $pageBtn .= "<li class='page-item'><a class='page-link' href='verDiplomas.php?&pagina=" . ($pagination->_current_page - 1) . "'> < </a></li>"; 
        } 
        for ($i = -4; $i <= 4; $i++) { 
            if($k + $i == $pagination->_current_page) {
                $pagLink .= "<li class='page-item active'><a class='page-link' href='verDiplomas.php?&pagina=" . ($k + $i) . "'>" . ($k + $i) . "</a></li>"; 
            } else {
                if ($k + $i > 0 && $k + $i <= $paginationData['total']) {
                    $pagLink .= "<li class='page-item'><a class='page-link' href='verDiplomas.php?&pagina=" . ($k + $i) . "'>" . ($k + $i) . "</a></li>";
                }
            }
        ...
        if($pagination->_current_page < $paginationData['total']){ 
            $pageBtn .= "<li class='page-item'><a class='page-link' href='verDiplomas.php?&pagina=" . ($pagination->_current_page + 1) . "'> > </a></li>"; 
            $pageBtn .= "<li class='page-item'><a class='page-link' href='verDiplomas.php?&pagina=" . $paginationData['total'] . "'> >> </a></li>"; 
        }
