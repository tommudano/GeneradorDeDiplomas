...
if(!$user->isLoggedIn()) {
    ...
    if(isset($_POST['eliminar']) && $user->data()->privilegio == "admin") {
        try {
            $curso->deleteById($_POST['id'], "cursos");
            Session::flash('consultaCurso', 'success', 'El curso fue eliminado con &eacute;xito');
            Redirect::to("consultarCurso.php");
        } catch (Exception $e) {
            Session::flash('consultaCurso', 'danger', $e->getMessage());
            Redirect::to("consultarCurso.php");
            die();
        }
    }
...
function loadPage() {
    ...
    $msjTag = Session::message('consultaCurso');
    $token = Token::generate();

    $resultsCount = $curso->contarTotal('cursos');
    $per_page = 3;
    ...
    $resultsToCast = $curso->getOffLim($pagination->offset(), $pagination->_per_page, 'cursos');
    $resultsToTemp = Curso::arrConInfo($resultsToCast);
    ...
    $paginaTodas = <<<CURSO
        ...
        <body>
            ...
            <div class="container mt-5">
                <h1 class="text-center">Cursos Creados</h1>
                {$msjTag}
                <div>
                    {$results}
                </div>
...
CURSO;
    ...
    if ($paginationData['total'] > 0){
        ...
        if($pagination->_current_page >= 2){ 
            $pageBtn .= "<li class='page-item'><a class='page-link' href='consultarCurso.php?&pagina=1'> << </a></li>"; 
            $pageBtn .= "<li class='page-item'><a class='page-link' href='consultarCurso.php?&pagina=" . ($pagination->_current_page - 1) . "'> < </a></li>"; 
        } 
        for ($i = -4; $i <= 4; $i++) { 
            if($k + $i == $pagination->_current_page) {
                $pagLink .= "<li class='page-item active'><a class='page-link' href='consultarCurso.php?&pagina=" . ($k + $i) . "'>" . ($k + $i) . "</a></li>"; 
            } else {
                if ($k + $i > 0 && $k + $i <= $paginationData['total']) {
                    $pagLink .= "<li class='page-item'><a class='page-link' href='consultarCurso.php?&pagina=" . ($k + $i) . "'>" . ($k + $i) . "</a></li>";
                }
            }
        ...
        if($pagination->_current_page < $paginationData['total']){ 
            $pageBtn .= "<li class='page-item'><a class='page-link' href='consultarCurso.php?&pagina=" . ($pagination->_current_page + 1) . "'> > </a></li>"; 
            $pageBtn .= "<li class='page-item'><a class='page-link' href='consultarCurso.php?&pagina=" . $paginationData['total'] . "'> >> </a></li>"; 
        }
