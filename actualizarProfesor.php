...
if (Input::exists()) {
    if(Token::check(Input::get('token'))) {
        ...
        $validation = $validate->check($_POST, array(
            ...
            'dni' => array(
                'required' => true,
                'uniqueExcept' => array('profesores', Input::get('id')),
                'typeValue' => 'int'
            ),
            'apellido' => array(
                'required' => true,
        ...
        if($validation->passed()) {
            ...
            try {
                ...
                if(file_exists($img['tmp_name']) && is_uploaded_file($img['tmp_name'])){
                    ...
                    $fields = array_merge($fields, array('firma' => $img64));
                }
            
                $profesor->actualizarPorId('profesores', escape(Input::get('id')), $fields);

                Session::flash('profesorActualizado', 'success', '¡El profesor ha sido actualizado correctamente!');
            } catch(Exception $e) {
                Session::flash('profesorActualizado', 'danger', $e->getMessage());
            }
        } else {
...
function loadPage() {
    ...
    $msjTag = Session::message('profesorActualizado');

    $idProfesor = escape(Input::get('id'));
    $nombreProfesor = escape(Input::get('nombreV'));
    $apellidoProfesor = escape(Input::get('apellidoV'));
    $dniProfesor = escape(Input::get('dniV'));
    ...
    $used[0] = $used[0] == '' ? $dniProfesor : $used[0];
    $used[1] = $used[1] == '' ? $nombreProfesor : $used[1];
    $used[2] = $used[2] == '' ? $apellidoProfesor : $used[2];
            ...
            <div class="container mt-5">
                ...
                <div class="col-md-12 mx-auto">
                    <form action="" method="POST" id="form" autocomplete="off" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="apellido">Ingrese el Apellido <span style="color:red;">*</span></label>
                        ...
                        <input type='hidden' name='nombreV' value='{$nombreProfesor}'>
                        <input type='hidden' name='apellidoV' value='{$apellidoProfesor}'>
                        <input type='hidden' name='dniV' value='{$dniProfesor}'>
                        <input type="hidden" name="token" value="{$token}">
                        <button type="submit" class="btn btn-danger" id="btn" disabled>Actualizar Profesor</button>
                        <a href="./consultarProfesor.php" class="btn btn-warning">Volver</a>
                    </form>
                </div>
            </div>
        </body>
